    ...
    <div style={{ maxWidth: 520, display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ padding: 16, borderRadius: 12, background: "var(--bg-card)", border: "1px solid var(--border-light)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: "var(--fs-14)", fontWeight: 600, color: "var(--text-primary)" }}>联网搜索</div>
            <div style={{ fontSize: "var(--fs-11)", color: "var(--text-muted)", marginTop: 2 }}>让 AI 可以在对话中搜索互联网获取最新信息</div>
          </div>
          <button onClick={toggleEnabled}
            style={{ width: 44, height: 24, borderRadius: 12, border: "none", padding: 0, cursor: "pointer", flexShrink: 0, background: enabled ? "var(--accent)" : "var(--border-medium)", position: "relative", transition: "background 0.2s ease" }}>
        ...
        {enabled && (
          <>
            <div style={{ fontSize: "var(--fs-11)", color: "var(--text-muted)", lineHeight: 1.5, marginBottom: 16, padding: "8px 12px", background: "var(--bg-hover)", borderRadius: 8 }}>
              启用后 AI 在需要时会自动调用联网搜索获取最新信息。选择下方的搜索服务并配置 API Key（DuckDuckGo 免费免配置）。
            </div>

            {/* Provider selector */}
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: "var(--fs-11)", fontWeight: 500, color: "var(--text-secondary)", display: "block", marginBottom: 6 }}>搜索服务</label>
                ...
                {PROVIDER_KEYS.map((k) => {
                  ...
                  return (
                    <button key={k} onClick={() => changeProvider(k)}
                      style={{ padding: "8px 4px", borderRadius: 8, cursor: "pointer", fontSize: "var(--fs-11)", fontWeight: 500, textAlign: "center", background: isActive ? "var(--accent-bg)" : "var(--bg-hover)", border: isActive ? "1px solid var(--accent-border)" : "1px solid var(--border-light)", color: isActive ? "var(--accent)" : "var(--text-secondary)", transition: "all 0.12s" }}>
                      <Wifi size={12} style={{ display: "block", margin: "0 auto 3px", color: isActive ? "var(--accent)" : "var(--text-muted)" }} />
                      {p.name}
                    </button>
                  );
                })}
            ...
            {provider !== "duckduckgo" && (
              <div>
                <label style={{ fontSize: "var(--fs-11)", fontWeight: 500, color: "var(--text-secondary)", display: "block", marginBottom: 6 }}>
                  {cfg.keyLabel}
                </label>
                <div style={{ display: "flex", gap: 6 }}>
                  ...
                  <a href={cfg.signupUrl} target="_blank" rel="noreferrer"
                    style={{ display: "flex", alignItems: "center", gap: 4, padding: "0 10px", borderRadius: 8, fontSize: "var(--fs-11)", color: "var(--accent)", textDecoration: "none", border: "1px solid var(--accent-border)", background: "var(--accent-bg)", whiteSpace: "nowrap" }}>
                    <ExternalLink size={11} /> 注册
                  </a>
                </div>
                <div style={{ fontSize: "var(--fs-10)", color: "var(--text-muted)", marginTop: 4 }}>{cfg.docs}</div>
              </div>
            )}

            {provider === "duckduckgo" && (
              <div style={{ padding: "8px 12px", borderRadius: 8, background: "var(--bg-hover)", border: "1px solid var(--border-light)", fontSize: "var(--fs-11)", color: "var(--text-muted)" }}>
      ...
      <div style={{ padding: "10px 14px", borderRadius: 8, background: "var(--bg-hover)", border: "1px solid var(--border-light)", fontSize: "var(--fs-11)", color: "var(--text-muted)", lineHeight: 1.5 }}>
        <strong style={{ color: "var(--text-secondary)" }}>工作原理：</strong>AI 判断需要搜索时自动调用 web_search 工具，将搜索结果整合到回复中。支持 5 种搜索引擎，可在上方切换。
      </div>
    </div>
  );
}
