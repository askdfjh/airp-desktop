  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "12px 14px 8px", borderBottom: "1px solid var(--border-light)" }}>
        ...
        {/* Tabs */}
        <div style={{ display: "flex", gap: 2, marginBottom: 8, background: "var(--bg-input)", borderRadius: 8, padding: 3 }}>
          <button
            ...
            style={{
              ...
              fontSize: "var(--fs-11)", fontWeight: 500,
              background: activeTab === "characters" ? "var(--accent)" : "transparent",
              color: activeTab === "characters" ? "#fff" : "var(--text-muted)",
          ...
          <button
            ...
            style={{
              ...
              padding: "5px 10px", borderRadius: 6,
              fontSize: "var(--fs-11)", fontWeight: 500,
              background: activeTab === "cards" ? "var(--accent)" : "transparent",
      ...
      {showNewForm && activeTab === "characters" && (
        <div style={{ padding: 10, borderBottom: "1px solid var(--border-light)", background: "var(--bg-card)", maxHeight: 260, overflowY: "auto" }}>
          <input value={newCharName} onChange={(e) => setNewCharName(e.target.value)} placeholder="角色名称 *"
            style={{ width: "100%", padding: "6px 10px", borderRadius: 6, background: "var(--bg-input)", border: "1px solid var(--border-light)", color: "var(--text-primary)", fontSize: "var(--fs-12)", fontFamily: "inherit", marginBottom: 6, outline: "none" }}
          />
          <input value={newCharAppearance} onChange={(e) => setNewCharAppearance(e.target.value)} placeholder="外貌描述"
            style={{ width: "100%", padding: "6px 10px", borderRadius: 6, background: "var(--bg-input)", border: "1px solid var(--border-light)", color: "var(--text-primary)", fontSize: "var(--fs-11)", fontFamily: "inherit", marginBottom: 6, outline: "none" }}
          />
          ...
          <textarea value={newCharBackground} onChange={(e) => setNewCharBackground(e.target.value)} placeholder="背景故事"
            style={{ width: "100%", padding: "8px 10px", borderRadius: 6, background: "var(--bg-input)", border: "1px solid var(--border-light)", color: "var(--text-primary)", fontSize: "var(--fs-11)", fontFamily: "inherit", minHeight: 50, resize: "vertical", marginBottom: 6, outline: "none", lineHeight: 1.4 }}
          />
          <input value={newCharTags} onChange={(e) => setNewCharTags(e.target.value)} placeholder="标签（用逗号分隔）"
            style={{ width: "100%", padding: "4px 8px", borderRadius: 6, background: "var(--bg-input)", border: "1px solid var(--border-light)", color: "var(--text-primary)", fontSize: "var(--fs-11)", fontFamily: "inherit", marginBottom: 6, outline: "none" }}
          />
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 6 }}>
            ...
            <button onClick={handleCreateCharacter} disabled={!newCharName.trim()} className="cp"
              style={{ padding: "5px 14px", borderRadius: 6, fontSize: "var(--fs-11)", fontWeight: 500, background: "var(--accent)", color: "#fff", border: "none", opacity: !newCharName.trim() ? 0.5 : 1, cursor: !newCharName.trim() ? "not-allowed" : "pointer" }}>
              创建角色
            </button>
          </div>
        </div>
      ...
      {showNewForm && activeTab === "cards" && (
        <div style={{ padding: 10, borderBottom: "1px solid var(--border-light)", background: "var(--bg-card)", maxHeight: 220, overflowY: "auto" }}>
          <div style={{ display: "flex", gap: 6, marginBottom: 6 }}>
            <input value={newEmoji} onChange={(e) => setNewEmoji(e.target.value)} placeholder="🎭" maxLength={4}
              style={{ width: 50, padding: "6px 8px", borderRadius: 6, background: "var(--bg-input)", border: "1px solid var(--border-light)", color: "var(--text-primary)", fontSize: 18, textAlign: "center", outline: "none" }}
            />
            <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="角色卡名称"
              style={{ flex: 1, padding: "6px 10px", borderRadius: 6, background: "var(--bg-input)", border: "1px solid var(--border-light)", color: "var(--text-primary)", fontSize: "var(--fs-12)", fontFamily: "inherit", outline: "none" }}
            />
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 6 }}>
            ...
            <button onClick={handleCreateCard} disabled={!newName.trim() || !newPrompt.trim()} className="cp"
              style={{ padding: "5px 14px", borderRadius: 6, fontSize: "var(--fs-11)", fontWeight: 500, background: "var(--accent)", color: "#fff", border: "none", opacity: !newName.trim() || !newPrompt.trim() ? 0.5 : 1, cursor: !newName.trim() || !newPrompt.trim() ? "not-allowed" : "pointer" }}>
              创建
            </button>
          </div>
        </div>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 2, flexShrink: 0 }}>
                    {c.isBuiltin && (
                      <span style={{ fontSize: "var(--fs-8)", padding: "0 3px", borderRadius: 2, background: "var(--bg-hover)", color: "var(--text-muted)", fontWeight: 500 }}>
                        内置
                      </span>
                    )}
                  </div>
                ...
                {c.appearance && (
                  <div style={{ fontSize: "var(--fs-10)", color: "var(--text-muted)", lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
                    <span style={{ color: "var(--text-tertiary)", fontWeight: 500 }}>外貌：</span>{c.appearance}
                  </div>
                )}
                {c.personality && (
                  <div style={{ fontSize: "var(--fs-10)", color: "var(--text-muted)", lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
                    <span style={{ color: "var(--text-tertiary)", fontWeight: 500 }}>性格：</span>{c.personality}
                  </div>
                )}
                {c.tags.length > 0 && (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 3, marginTop: 2 }}>
                    {c.tags.slice(0, 3).map((tag) => (
                      <span key={tag} style={{ fontSize: "var(--fs-9)", padding: "1px 5px", borderRadius: 3, background: "var(--accent-bg)", color: "var(--accent)", fontWeight: 500 }}>
                        {tag}
                      </span>
                    ))}
                  </div>
              ...
              >
                {editingId === c.id ? (
                  <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    <div style={{ display: "flex", gap: 4 }}>
                      <input value={editEmoji} onChange={(e) => setEditEmoji(e.target.value)} maxLength={4}
                        style={{ width: 40, padding: "4px 6px", borderRadius: 5, background: "var(--bg-input)", border: "1px solid var(--accent-border)", color: "var(--text-primary)", fontSize: 16, textAlign: "center", outline: "none" }}
                      />
                      <input value={editName} onChange={(e) => setEditName(e.target.value)}
                        style={{ flex: 1, padding: "4px 8px", borderRadius: 5, background: "var(--bg-input)", border: "1px solid var(--accent-border)", color: "var(--text-primary)", fontSize: "var(--fs-12)", fontFamily: "inherit", outline: "none" }}
                      />
                    </div>
                    <input value={editDesc} onChange={(e) => setEditDesc(e.target.value)}
                      style={{ width: "100%", padding: "4px 8px", borderRadius: 5, background: "var(--bg-input)", border: "1px solid var(--accent-border)", color: "var(--text-primary)", fontSize: "var(--fs-11)", fontFamily: "inherit", outline: "none" }}
                    />
                    <textarea value={editPrompt} onChange={(e) => setEditPrompt(e.target.value)}
                      style={{ width: "100%", padding: "5px 8px", borderRadius: 5, background: "var(--bg-input)", border: "1px solid var(--accent-border)", color: "var(--text-primary)", fontSize: "var(--fs-11)", fontFamily: "inherit", minHeight: 50, resize: "vertical", outline: "none", lineHeight: 1.4 }}
                    />
                    <input value={editTags} onChange={(e) => setEditTags(e.target.value)}
                      placeholder="标签(逗号分隔)"
                      style={{ width: "100%", padding: "3px 6px", borderRadius: 5, background: "var(--bg-input)", border: "1px solid var(--accent-border)", color: "var(--text-primary)", fontSize: "var(--fs-10)", fontFamily: "inherit", outline: "none" }}
                    />
                    <div style={{ display: "flex", justifyContent: "flex-end", gap: 4 }}>
                      <button onClick={() => setEditingId(null)} className="cp"
                        style={{ padding: "3px 10px", borderRadius: 5, fontSize: "var(--fs-11)", color: "var(--text-tertiary)", background: "transparent", border: "1px solid var(--border-light)" }}>
                        取消
                      </button>
                      <button onClick={saveEditCard} className="cp"
                        style={{ padding: "3px 10px", borderRadius: 5, fontSize: "var(--fs-11)", fontWeight: 500, background: "var(--accent)", color: "#fff", border: "none", display: "flex", alignItems: "center", gap: 2 }}>
                        <Check size={10} /> 保存
                      </button>
                    </div>
                  </div>
                  ...
                  <>
                        ...
                        {!c.isBuiltin && (
                          <>
                            <button onClick={(e) => { e.stopPropagation(); startEditCard(c.id); }} className="cp" title="编辑"
                              style={{ width: 20, height: 20, borderRadius: 4, background: "transparent", color: "var(--text-muted)", border: "none", display: "flex", alignItems: "center", justifyContent: "center" }}
                    ...
                    {c.description && (
                      <div style={{ fontSize: "var(--fs-11)", color: "var(--text-muted)", lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
                        {c.description}
                      </div>
                    )}
                    {c.tags.length > 0 && (
